...
from src.v20.runtime import build_v20_live_bundle, build_v20_local_judge, build_v20_runtime_state
...
def _judge_comparison_v20(kimi_judge: Mapping[str, Any], local_judge: Mapping[str, Any], runtime: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": f"Kimi and Local V20 both read {kimi_call}." if agree else f"Kimi is {kimi_call} while Local V20 is {local_call}.",
        "reasoning": "Both judges align on the current 15-minute bar." if agree else "The two judges disagree, which is useful for live accuracy comparison.",
...
def _build_v20_forecast(payload: Mapping[str, Any], runtime: Mapping[str, Any]) -> dict[str, Any]:
    candles = list((((payload.get("market") or {}).get("candles")) or []))
    if not candles:
        return {"points": []}
    base_timestamp = candles[-1].get("timestamp")
    base = Path()
    del base  # keep lint quiet for Windows path parser bug in some environments
    import pandas as pd
...
def create_app_v20() -> FastAPI:
    app = FastAPI(title="Nexus Trader V20 API", version="0.20.0")
    frontend_served = False
    if FRONTEND_DIST_PATH.exists():
        app.mount("/ui", StaticFiles(directory=str(FRONTEND_DIST_PATH), html=True), name="nexus_ui_v20")
    ...
    def _decorate_local_v20(payload: dict[str, Any], local_judge: dict[str, Any]) -> dict[str, Any]:
        decorated = _decorate_kimi_judge(payload, local_judge)
        decorated["judge_name"] = "local_v20"
        decorated["provider"] = "local_sjd_v20"
        decorated["model"] = "v20_rule_judge"
        return decorated
    ...
    def _dashboard_payload(symbol: str, mode: str, llm_provider: str, llm_model: str | None, *, with_kimi: bool = False, force_kimi: bool = False) -> dict[str, Any]:
        ...
        local_judge = _decorate_local_v20(payload, build_v20_local_judge(payload, bundle))
        runtime = build_v20_runtime_state(payload, mode=mode)
        simulation = dict(payload.get("simulation", {}))
        simulation.update(
            {
                "direction": runtime.get("decision_direction", simulation.get("direction", "HOLD")),
                "confidence_tier": runtime.get("confidence_tier", simulation.get("confidence_tier", "uncertain")),
                ...
                "hurst_overall": (bundle.feature_row.get("hurst_overall") if bundle is not None else simulation.get("hurst_overall")),
                "hurst_asymmetry": (bundle.feature_row.get("hurst_asymmetry") if bundle is not None else simulation.get("hurst_asymmetry")),
                "detected_regime": (bundle.feature_row.get("hmm_state_name") if bundle is not None else simulation.get("detected_regime")),
                "branch_count": 64,
            }
        )
        ...
        payload["final_forecast"] = _build_v20_forecast(payload, runtime)
        payload["stack_mode"] = "v20_live_dual_judge"
        ...
        payload["judge_comparison"] = _judge_comparison_v20(payload["kimi_judge"], local_judge, runtime)
        return payload

    @app.get("/health")
    def health():
        return {"status": "ok", "version": "v20", "host": "app_v20"}
    ...
    def kimi_live(symbol: str = "XAUUSD", llm_provider: str = "nvidia_nim", llm_model: str | None = None, mode: str = "frequency", force: bool = False):
        ...
        return {
            ...
            "v20_runtime": payload.get("v20_runtime", {}),
            "v19_runtime": payload.get("v20_runtime", {}),
        }
    ...
    async def startup_tasks():
        if getattr(app.state, "v20_feed_task", None) is None:
            app.state.v20_feed_task = asyncio.create_task(feed_manager.heartbeat_loop(price_fn=_live_price, sqt_fn=lambda: {"label": "V20"}))

    @app.on_event("shutdown")
    async def shutdown_tasks():
        task = getattr(app.state, "v20_feed_task", None)
        if task is not None:
            task.cancel()
            app.state.v20_feed_task = None
