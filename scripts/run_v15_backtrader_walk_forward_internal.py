...
from config.project_config import (
    ...
    V15_BACKTRADER_WALKFORWARD_REPORT_PATH,
...
def _report_path_for_month(month: str) -> Path:
    if month == '2023-12':
        return PROJECT_ROOT / 'outputs' / 'v15' / 'backtrader_month_2023_12_v15.json'
    return PROJECT_ROOT / 'outputs' / 'v15' / f'backtrader_month_{month.replace("-", "_")}_v15.json'
...
def _month_pass(report: dict) -> bool:
    ...
    return 15 <= trades <= 50 and win_rate > 0.58 and profit_factor > 2.0 and max_dd < 12.0 and gap < 0.05
...
def _failure_reasons(report: dict) -> list[str]:
    ...
    if not (float(report.get('win_rate', 0.0)) > 0.58):
...
def _load_rsc_summary() -> dict:
    for path in (V15_RSC_RUNTIME_PATH, V15_RSC_BOOTSTRAPPED_PATH):
        if path.exists():
            return RegimeStratifiedCalibrator.load(path).summary()
    return RegimeStratifiedCalibrator().summary()
...
def main() -> int:
    ...
    final_report = {
        ...
        'avg_pce_rejection_rate': round(float(np.mean([float(report.get('pce_rejection_rate', 0.0)) for report in valid_reports])) if valid_reports else 0.0, 6),
        'paper_trade_summary': paper_summary,
        'rsc_summary': _load_rsc_summary(),
    ...
    V15_BACKTRADER_WALKFORWARD_REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    V15_BACKTRADER_WALKFORWARD_REPORT_PATH.write_text(json.dumps(final_report, indent=2), encoding='utf-8')
    print(str(V15_BACKTRADER_WALKFORWARD_REPORT_PATH), flush=True)
